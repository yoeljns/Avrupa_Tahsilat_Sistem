...
// matrisiyle, yaşlandırma şeridi eklenerek sunulur. Yaşlandırma da
...
export default async function PesinPage({ searchParams }: { searchParams: Promise<{ ay?: string }> }) {
  ...
  let veri: MatrisAy
  try {
    veri = await matrisAy(supabase, 'PESIN', month)
  } catch (e) {
      ...
      <div className="flex flex-wrap items-center justify-between gap-3">
        <div>
          ...
          <p className="mt-1 text-sm text-slate-500">
            ...
            <strong className="tabular-nums text-emerald-600">{eur(o?.toplam_odenen ?? 0)}</strong> · Kalan:{' '}
            <strong className="tabular-nums">{eur(o?.toplam_kalan ?? 0)}</strong>
          </p>
        </div>
